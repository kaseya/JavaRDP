...
// static boolean pstcache_load_bitmap(int cache_id, int cache_idx, Options option) throws IOException, RdesktopException
// {
...
//    byte[] cellHead = null;
//
//    if (!option.isPersistentBitmapCachingEnabled())
//        return false;
//
//    if (!IS_PERSISTENT(cache_id) || cache_idx >= Rdp.BMPCACHE2_NUM_PSTCELLS)
//        return false;
...
//
//        bitmap = new Bitmap(celldata,c.width,c.height,0,0, option.getColourDepthInBytes(), option);
...
// }
 ...
 static boolean pstcache_put_bitmap(int cache_id, int cache_idx, byte[] bitmap_id, int width, int height, int length, byte[] data, Options option) throws IOException
 {
    ...
    cellhdr.stamp = 0;
        ...
        fd = new FileOutputStream(g_pstcache_fd[cache_id]);
        int offset = cache_idx * (option.getColourDepthInBytes() * MAX_CELL_SIZE + CELLHEADER.size());
...
// static int pstcache_enumerate(int cache_id, int[] idlist, Options option) throws IOException, RdesktopException
// {
...
//
//    if (!(option.isBitmapCachingEnabled() && option.isPersistentBitmapCachingEnabled() && IS_PERSISTENT(cache_id)))
//        return 0;
//
...
//                if (option.isPrecacheBitmaps() && (option.isServerDisplayColourDepthGreaterThanEightBits()))
//                {
//                    if (pstcache_load_bitmap(cache_id, n, option))
//                        c++;
//                }
//
...
// }
 ...
 static boolean pstcache_init(int cache_id, Options option)
 {
    //int fd;
    String filename;
    ...
    if (!(option.isBitmapCachingEnabled() && option.isPersistentBitmapCachingEnabled()))
        return false;

    g_pstcache_Bpp = option.getColourDepthInBytes();
