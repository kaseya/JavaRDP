...
public class MapDef {
	...
	private final int FLAG_CAPSLOCK = 0x08; // flag mask for a capslock modifier
	  ...
	  protected boolean appliesToPressed(KeyEvent e){
	  	...
	  	if(!characterDef){
	  		if(!((ctrlDown && e.isControlDown()) || !ctrlDown)) return false;
	  		if(!((altDown && e.isAltDown()) || !altDown)) return false;
	  	}
	  ...
	  public MapDef(String definition) throws KeyMapException{
	  	...
	  	try{
	  			...
	  			if(characterDef) keyChar = (char) Integer.parseInt(st.nextToken());
	  			else keyCode = Integer.parseInt(st.nextToken());
	  ...
	  public void writeToStream(PrintStream p){
	    ...
	    // add key location
	  	...
	  	if(!characterDef) definition += "\t" + KeyEvent.getKeyText(this.keyCode);
